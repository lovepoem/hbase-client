...
public class HbaseConnectionFactory implements ConnectionFactory<Connection> {
     ...
     * <p>Description: 构造方法</p>
     *
     * @param zkQuorum zk集群地址
     * @param zkClientPort zk客户端
     */
    public HbaseConnectionFactory(final String zkQuorum,final String zkClientPort) {
        Configuration hadoopConfiguration = new Configuration();
        hadoopConfiguration.set(HConstants.ZOOKEEPER_QUORUM, zkQuorum);
        hadoopConfiguration.set(HConstants.ZOOKEEPER_CLIENT_PORT, zkClientPort);
        this.hadoopConfiguration = hadoopConfiguration;
    }
    ...
    public HbaseConnectionFactory(final String host, final String port, final String master, final String rootdir) {
        ...
        if (master != null) this.hadoopConfiguration.set(HbaseConfig.MASTER_PROPERTY, master);
        if (rootdir != null) this.hadoopConfiguration.set(HbaseConfig.ROOTDIR_PROPERTY, rootdir);
    }
    ...
    public boolean validateObject(PooledObject<Connection> p) {
        Connection connection = p.getObject();
        if (connection != null) return ((!connection.isAborted()) && (!connection.isClosed()));
        return false;
    }
